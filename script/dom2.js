...
console.log(header);
// tag, class 등과 같이 다수가 존재할 수 있는 대상을 DOM으로 선택하면 특정 대상 1개만 잡히는게 아닌 Collection 기준으로 여러개가 잡히기 때문에 원하는 대상 사용 시 정확히 그 대상을 인덱스로 선택해야 한다.
...
/*
    ...
    대신,인덱스가 고정되므로 다른 인덱스번호에 해당하는 태그나 클래스를 사용하고
    싶을 때 변수를 새로 만들어야 한다.
    console.log(aTag2);
...
function btn_func4(){
    ...
    let order_num = Number(input_num.value);
    let price = 1000
