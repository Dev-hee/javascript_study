...
login_btn.addEventListener('click',()=>{
    ...
    if(user_id.value == "admin"){
        // == > < != >= <=
        // input 의 값이 빈문자열이 아닐때 참
        // console.log("input에 값이 작성되어있다")
        alert(`관리자 ${user_id.value}님 어서오세요`)
    }else{
        alert(`회원님 ${user_id.value}님 어서오세요`)
    }
})
console.log('----------------')
...
// hint : 짝수구분 나머지 연산자로 2를 나누 나머지 값 체크하기
...
walk_btn.addEventListener('click',()=>{
    if(walk.value >= 5000){
        alert(`목표 달성 ${walk.value}보 걸으셨네요 !!`)
    }else{
...
print_btn.addEventListener('click',()=>{
    ...
    if(result){ 
        //참에 대한 코드블록 {}
        //인쇄할까요? 에서 '확인' 을 클릭해서 true를 반환 받았을 때 if문이 실행되는 결과 만들기
        console.log('.')
        print()
    } else{
        //조건거짓에 대한 코드블록 위치{}
